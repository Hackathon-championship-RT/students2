...
const generateCards = (layerIndex, gridSize) => {
  ...
  const images = require.context('./electric', false, /\.(jpg|jpeg|png)$/);
...
// Main function
const MahjongElectricCarsEasy1 = () => {
  const gridSize = 2;
  const numLayers = 2;
  ...
  const checkLevelCompletion = useCallback(() => {
    ...
    if (allMatched) {
      ...
      navigate('/mahjong-electricCars-easy-2');
    }
  ...
  const sendCoinsToServer = async (coins) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/records', {
        ...
        body: JSON.stringify({ "level": 0, "score": coins }),
      });
...
export default MahjongElectricCarsEasy1;
